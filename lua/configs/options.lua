...
-- o.completion_enable_fuzzy_match = 1
-- o.diagnostic_enable_virtual_text = 1
-- o.diagnostic_show_sign = 1
-- o.diagnostic_insert_delay = 1

-- map('', <tab> <Plug>(completion_smart_tab)
-- map('', <s-tab> <Plug>(completion_smart_s_tab)

--  highlight yanks in neovim
-- au TextYankPost * silent! lua require'highlight'.on_yank("IncSearch", 800, vim.v.event)
vim.cmd 'au TextYankPost * lua vim.highlight.on_yank {on_visual = false}'  -- disabled in visual mode

--  augroup ~/vimwiki/*,~/todo/todo.md,~/.config/nvim/*,~/AppData/Local/nvim/*
--    autocmd!
--    autocmd BufReadPre ~/vimwiki/*,~/todo/todo.md,~/.config/nvim/*,~/AppData/Local/nvim/* call AutoGit("pull")
--    autocmd BufWritePost ~/vimwiki/*,~/todo/todo.md,~/.config/nvim/*,~/AppData/Local/nvim/* call AutoGit("push")
--  augroup END
